...
namespace UnityCliConnector.Tools
{
    ...
    public static class ExecuteCsharp
    {
            ...
            try
            {
                ...
                if (exe == null)
                    return new ErrorResponse(
                        "Cannot find csc compiler. Searched: " +
                        Path.Combine(EditorApplication.applicationContentsPath, "DotNetSdkRoslyn") + ", " +
                        Path.Combine(EditorApplication.applicationContentsPath, "Tools", "RoslynScripts"));
        ...
        private static (string exe, string args) FindCsc(string rspFile)
        {
            ...
            // Unity 6+: DotNetSdkRoslyn
            // Unity 2022 / 2021: Tools/RoslynScripts
            var candidateDirs = new[]
            {
                Path.Combine(content, "DotNetSdkRoslyn"),
                Path.Combine(content, "Tools", "RoslynScripts"),
            };

            foreach (var roslynDir in candidateDirs)
            {
